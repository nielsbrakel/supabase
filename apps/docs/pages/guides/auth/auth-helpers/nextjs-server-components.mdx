...
All Client Components need to share a single instance of the Supabase client in order to prevent auth session conflicts. We can wrap our application in a `<SupabaseProvider />` and use React Context to create a global Supabase instance.
